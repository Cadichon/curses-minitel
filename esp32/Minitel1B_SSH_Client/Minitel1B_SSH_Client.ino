...
const char* ssid = "hotspot";    // your wifi network
const char* password = "XXX";  // your wifi password

// ---------------------------------------
// ------ SSH session config.

const char* ssh_host = "192.168.X.X";     // host name or ip adress (e.g. "192.168.0.1")
const char* ssh_username = "you";  // user name (e.g. "pi")
const char* ssh_password = "password";      // user password (e.g. "raspberrypi")
...
void sshTask(void* pvParameters) {
  ...
  while (true) {
    ...
    switch (key) {
      ...
      case SOMMAIRE: key = 0x01; break;      // END : ctrl+A : start of line
      case GUIDE: key = 0x07; break;         // BEL : ring
      case ANNULATION: key = 0x0515; break;  // ctrl+A ctrl+Del : start of line + remove left
      case CORRECTION: key = 0x7F; break;    // DEL : delete
      case RETOUR: key = 0x05; break;        // ctrl+E : end of line
      case SUITE: key = '_'; break;          // _
